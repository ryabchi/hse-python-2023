...
class Team:
    ...
    def __str__(self):
        """
        Функция возвращает строку, которая содержит:
        -название команды (self.name),
        -имя менеджера команды (self.manager.name)
        -количество участников в команде (len(self.__members))
        теперь можно использовать print(self) :)
        """
