...
class Team:
    ...
    def remove_member(self, member: Employee) -> None:
        ...
        if not isinstance(member, Employee):
            raise TypeError
        for index, empl in enumerate(self.__members):
            if empl.name == member.name:
                self.__members.remove(empl)
                return
    ...
    def __str__(self):
        result = 'team: ' + self.name + ' manager: ' + self.manager.name + ' number of members: ' + str(len(self.__members))
        return result
    def show(self) -> None:
        """
        DO NOT EDIT!
