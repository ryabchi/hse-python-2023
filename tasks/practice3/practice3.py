...
def count_words(text: str) -> Dict[str, int]:
    ...
    resulted_dict = {word: text.count(word) for word in text if word.isalpha() and len(word) > 1}
...
def get_cashback(operations: List[Dict[str, Any]], special_category: List[str]) -> float:
    ...
    return sum(op['amount'] * (0.05 if op['category'] in special_category else 0.01)
...
def csv_reader(header: str) -> int:
    ...
    with open(get_path_to_file(), 'r', newline='') as csv_file:
        reader = csv.reader(csv_file)
        column_index = next(reader).index(header)
        unique_values = set(row[column_index] for row in reader)
