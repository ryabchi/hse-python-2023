...
def moderate_text(text: str, uncultured_words: Iterable[str]) -> str:
    ...
    # пиши код здесь
    new_text_words = re.sub('[\']|[\"]', '', re.sub(' +', ' ', text)).lower().split()
    new_text_words[0] = new_text_words[0].title()
    for i in range(len(new_text_words)):
        for uncultured_word in uncultured_words:
            if new_text_words[i].startswith(uncultured_word):
                new_text_words[i] = new_text_words[i].replace(uncultured_word, "#" * len(uncultured_word))
