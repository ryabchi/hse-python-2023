...
def is_phone_correct(phone_number: str) -> bool:
    ...
    if re.compile("^((\+7)+([0-9]){10})$").fullmatch(phone_number):
        return True
    return False
...
def moderate_text(text: str, uncultured_words: Iterable[str]) -> str:
    ...
    for _ in uncultured_words:
        result = result.replace(_, '#' * len(_))
    return result
...
def create_request_for_loan(user_info: str) -> str:
    ...
    surname, name, second_name, date, amount = user_info.split(',')
