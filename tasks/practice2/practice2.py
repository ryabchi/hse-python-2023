...
def is_phone_correct(phone_number: str) -> bool:
    ...
    result = re.fullmatch("\\+7\\d{10}", phone_number) and len(phone_number) == 12
    return result
...
def moderate_text(text: str, uncultured_words: Iterable[str]) -> str:
    ...
    text_lower_filtered = text.replace("  ", ' ').replace("\"", '').replace("\'", '').lower().strip()

    for word in uncultured_words:
        text_lower_filtered = text_lower_filtered.replace(word, '#' * len(word))

    result = text_lower_filtered[0].upper() + text_lower_filtered[1:]
    return result
...
def create_request_for_loan(user_info: str) -> str:
    ...
    parsed_user_info = user_info.split(',')

    result = f"Фамилия: {parsed_user_info[0]}\n" \
             f"Имя: {parsed_user_info[1]}\n" \
             f"Отчество: {parsed_user_info[2]}\n" \
             f"Дата рождения: {parsed_user_info[3]}\n" \
             f"Запрошенная сумма: {parsed_user_info[4]}"
