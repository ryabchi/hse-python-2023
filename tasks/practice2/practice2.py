...
def moderate_text(text: str, uncultured_words: Iterable[str]) -> str:
    ...
    for word in text.split():
        ...
        for un_word in uncultured_words:
            if un_word in word:
                result_list.append(
                    "".join(['#' for i in un_word]) + "".join([word[i] for i in range(len(un_word), len(word))]))
                flag = True
                break
        if not flag:
...
def create_request_for_loan(user_info: str) -> str:
    ...
    result = f"Фамилия: {user_list[0]}\n" \
             f"Имя: {user_list[1]}\n" \
             f"Отчество: {user_list[2]}\n" \
             f"Дата рождения: {user_list[3]}\n" \
             f"Запрошенная сумма: {user_list[4]}"
    return result
