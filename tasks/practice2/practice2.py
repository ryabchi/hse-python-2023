...
def get_amount() -> float:
    ...
    amount = round(random.triangular(100, 1000000), 2)
...
def is_phone_correct(phone_number: str) -> bool:
    ...
    result = True if re.fullmatch(r'\+7[0-9]{10}', phone_number) else False
...
def moderate_text(text: str, uncultured_words: Iterable[str]) -> str:
    ...
    result = text
    result = result.strip().replace("'", "").replace('"', '').capitalize()
